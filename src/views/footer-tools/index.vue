    ...
    <template v-for="item in toolList" :key="item.key">
      ...
      <el-tooltip v-else effect="dark" :content="item.title" placement="top" popper-class="footer-tooltip">
        <div class="tool-box" @click="item.click(item)">
          <i v-if="item.key === 'fullScreen'"
            :class="['iconfont', isFullScreen ? 'icon-quanping' : 'icon-fullscreen-expand']"></i>
          <i v-else-if="item.key === 'showOrHideComment'"
            :class="['iconfont', isShowComment ? 'icon-eye' : 'icon-no_eye']"></i>
